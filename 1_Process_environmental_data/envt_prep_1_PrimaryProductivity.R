...
writeRasterTemp=function(file,outdir,var){
  ...
  savename200m=glue("{outdir}/{date}/{var}upper200m.grd")
  # savename_sd=glue("{outdir}/{date}/{var}_sd.grd")
  print(date)
  
  if(!file.exists(savename200m)){
    print("Primary productivity doesn't exist, writing out")
    if(date>="2017-01-01") {r=raster::brick(file,varname="nppv",lvar=4) %>% .[[1:26]] %>% mean(.,na.rm=T)}
    if(date<"2017-01-01") {r=raster::brick(file,varname="nppv",lvar=4) %>% .[[1:31]] %>% mean(.,na.rm=T)} 
...
mclapply(Files,FUN=writeRasterTemp,outdir=dailyDir,mc.cores=8,var="PP")
# lapply(Files,FUN=writeRasterTemp,outdir=dailyDir,var="PP")
